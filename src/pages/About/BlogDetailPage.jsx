      ...
      <div className="border-b border-gray-200 pb-8 last:border-0">
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-800 leading-relaxed mt-2">{comment.comment}</p>
            <button onClick={() => setShowReply(true)} className="mt-3 text-sm font-medium text-blue-600 hover:underline">
              Reply
            </button>
            {showReply && (
              <div className="mt-6 ml-10 p-5 bg-gray-50 rounded-2xl border">
                ...
                <div className="mt-4 flex gap-3">
                  <button onClick={() => {
                    ...
                    setReplyName("");
                    setReplyText("");
                  }} className="bg-blue-600 text-white px-6 py-2.5 rounded-lg hover:bg-blue-700 transition">Post Reply</button>
                  <button onClick={() => setShowReply(false)} className="text-gray-600 hover:text-black">Cancel</button>
                </div>
              </div>
            )}
            {comment.replies && comment.replies.length > 0 && (
              <div className="mt-8 ml-12 space-y-6 border-l-2 border-gray-200 pl-6">
                {comment.replies.map(reply => <CommentItem key={reply._id} comment={reply} addReply={addReply} />)}
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="comments-wrapper">
                    ...
                    <div className="comments-list">
                      {comments.length === 0 ? (
                        ...
                        comments.map(comment => <CommentItem key={comment._id} comment={comment} addReply={addReply} />)
                      )}
                    </div>
                  </div>
                )}
